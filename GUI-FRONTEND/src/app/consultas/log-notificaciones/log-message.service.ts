...
export class LogMessageService extends BaseService{
  ...
  getLogNotifByCriPag(criterios: any, direction: string, properties: string, page: string, size: string): Observable<any> {
    ...
    return this.http.post(this.appConfig.getConfig('URL_LOG_MESSAGE') + '/list',
  ...
  getReportXlsx(criterios: any, direction:any, properties:any){
    ...
    options.params =params;
    return this.http.post(this.appConfig.getConfig('URL_REPORTS') + "/logMessage/download/xlsx",JSON.stringify(criterios),options).map((res: Response) => res).catch(this.handleError.bind(this))
  }
